...
def _resolve_rgb_depth_paths(root, frame_info, frame_id):
    """Resolve RGB/depth relative paths from a frame entry with backward compatibility."""
    rgb_rel = frame_info.get('file_path') or frame_info.get('image_path')
    if not rgb_rel:
        raise KeyError(
            f"Frame {frame_id} missing RGB path field ('file_path'/'image_path')."
        )

    depth_rel = frame_info.get('depth_file_path') or frame_info.get('depth_path')
    if depth_rel:
        return rgb_rel, depth_rel

    rgb_dir = os.path.dirname(rgb_rel)
    rgb_name = os.path.basename(rgb_rel)
    rgb_stem, _ = os.path.splitext(rgb_name)

    candidates = []
    if rgb_stem.startswith("rgb_"):
        candidates.append(os.path.join(rgb_dir, rgb_stem.replace("rgb_", "depth_", 1) + ".png"))
    if rgb_stem.startswith("frame_"):
        candidates.append(os.path.join(rgb_dir, rgb_stem.replace("frame_", "depth_", 1) + ".png"))
    candidates.append(os.path.join(rgb_dir, f"depth_{rgb_stem}.png"))

    for rel_path in candidates:
        if os.path.exists(os.path.join(root, rel_path)):
            return rgb_rel, rel_path

    raise KeyError(
        "Frame "
        f"{frame_id} missing depth path field ('depth_file_path'/'depth_path'), "
        "and no inferred depth image was found. "
        "If this transforms.json was generated by a mono run, rerun Step1 without --mono."
    )
...
def unproj_depth(args):
    ...
    for frame_id, frame_info in enumerate(transforms['frames']):
        ...
        if args.correct_convention:
            RT.append(transform_matrix @ CAM_CONVENTION_CHANGE)
        else:
            RT.append(transform_matrix)

        rgb_rel, depth_rel = _resolve_rgb_depth_paths(root, frame_info, frame_id)
        rgb_paths.append(os.path.join(root, rgb_rel))
        depth_paths.append(os.path.join(root, depth_rel))
